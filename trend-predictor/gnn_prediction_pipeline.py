...
def load_artifacts(base_dir, device):
    model_path = os.path.join(base_dir, "saved_model_gnn", "dual_head_transformer.pt")
    scaler_path = os.path.join(base_dir, "saved_model_gnn", "scaler.pkl")
...
def run_prediction(asof_date):
    ...
    out_dir = os.path.join(base_dir, "saved_model_gnn", "predictions")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Predict next-day price and movement using GNN-trained model."
    )
    parser.add_argument(
        "--date",
        type=str,
